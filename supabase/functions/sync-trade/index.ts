...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!lockTriggered && newDailyLossR >= (Number(trade.tier_daily_loss_limit_r) || Number(tradingRules?.daily_loss_limit_r) || 3)) {
      await supabase.from('lock_events').insert({
    ...
    // --- EVALUATION METRICS + PROMOTION (mirrors close-trade) ---
    ...
    try {
      ...
      const metrics = {
        total_trades: totalCount, total_wins: wins, total_losses: losses,
      ...
      const evaluationTiers = (conf.evaluation_tiers as any);
      ...
      if (nextTier?.promotion) {
        ...
        if ((!p.min_win_rate || winRate >= p.min_win_rate)
          && (!p.min_trades || totalCount >= p.min_trades)
          && (!p.min_profit_factor || profitFactor >= p.min_profit_factor)
          && (!p.max_drawdown_pct || maxDd <= p.max_drawdown_pct)) {
          await supabase.from('profiles')
            .update({ evaluation_tier: nextTier.tier, updated_at: new Date().toISOString() })
            .eq('id', user.id);
          tierPromoted = {
            from_tier: currentTier, to_tier: nextTier.tier,
            from_name: tiersArr.find((t: any) => t.tier === currentTier)?.name || 'Unknown',
            to_name: nextTier.name,
            reason: `WR ${Math.round(winRate*100)}% · ${totalCount} trades`,
          };
        }
      }
    } catch (_) {}

    __log(200, undefined, { success: true, tradeId, symbol: trade.symbol, firedSide, exitPrice, pnl: Math.round(pnlUsdt * 100) / 100, tierPromoted });
    return json({
      success: true,
      stillOpen: false,
      firedSide,
