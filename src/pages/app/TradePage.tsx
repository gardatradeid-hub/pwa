...
export default function TradePage() {
  ...
  const handleExecute = async (side: 'long' | 'short', data: any) => { setError(null); setIsSubmitting(true); try { const result = await executeTrade({ ...data, side }); if (!result.success) { if (handleAuthError(result.error || '')) return; const msg = formatEdgeError(result); setError(msg); addToast('error', 'Trade Gagal', msg); return; } tradeStore.setActiveTrade(result.trade); tradeStore.setTradesToday(tradeStore.tradesToday + 1, phase.max_trades); addToast('success', 'Trade Terbuka', `${side.toUpperCase()} ${data.symbol} @ ${formatPrice(data.entryPrice)}`); } catch (e: any) { const msg = formatEdgeError(e); if (!handleAuthError(msg)) { setError(msg); addToast('error', 'Error', msg); } } finally { setIsSubmitting(false); } };
